...
function createTrayIconImage(imageName: string) {
  fullPath = path.join(__dirname, imageName);
...
function quitApp() {
  //if(mainWindow)mainWindow.setTitle(fullPath);
  isAppQuitting = true;
  app.quit();
}
  ...
  if (isLinux) {
    if (process.env.APPIMAGE) {
      ...
      outlineAutoLauncher.isEnabled()
          ...
          .catch((err: Error) => {
            console.error(`failed to add autolaunch entry for ${err.message}`);
          });
    }
  } else {
